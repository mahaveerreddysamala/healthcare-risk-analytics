...
def generate_report(path='data/heart_disease.csv', output_dir='reports'):
    out = Path(output_dir); out.mkdir(parents=True, exist_ok=True)
    df = profile(path)
    df['high_risk'].value_counts().sort_index().plot(kind='bar')
    plt.title('Healthcare Risk Class Distribution'); plt.xlabel('High Risk'); plt.ylabel('Patients')
    plt.tight_layout(); plt.savefig(out / 'risk_distribution.png', dpi=160); plt.close()
